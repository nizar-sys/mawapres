...
@section('title-header', 'Data Warna Produk')
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
    <li class="breadcrumb-item active">Data Warna Produk</li>
@endsection

@if (!in_array(Auth::user()->role, ['gudang']))

    @section('action_btn')
        <a href="{{ route('colors.create') }}" class="btn btn-default">Tambah Data</a>
    @endsection
@endif
        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header bg-transparent border-0 text-dark">
                    <h2 class="card-title h3">Data Warna Produk</h2>
                                        ...
                                        @if (!in_array(Auth::user()->role, ['gudang']))
                                            <td class="d-flex jutify-content-center">
                                                ...
                                                <form id="delete-form-{{ $color->id }}"
                                                    action="{{ route('colors.destroy', $color->id) }}" class="d-none"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                                <button onclick="deleteForm('{{ $color->id }}')"
